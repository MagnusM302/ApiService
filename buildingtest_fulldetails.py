...
class TestBuildingService(unittest.TestCase):
    ...
    def test_get_address_details_with_valid_token(self):
        ...
        address_details_url = f"{self.base_url}_details"
        response = requests.post(address_details_url, headers=self.headers, json={"address": self.address})
        print(f"Details Response Status Code: {response.status_code}")
        print(f"Details Response Data: {response.json()}")
        self.assertEqual(response.status_code, 200)
        self.assertIn('id', response.json())
        self.assertIn('adgangsadresse', response.json())
        
    def test_get_address_success(self):
        print("Testing get_address_success...")
        response = requests.post(self.base_url, headers=self.headers, json={"address": self.address})
    ...
    def test_get_address_unauthorized(self):
        ...
        response = requests.post(self.base_url, json={"address": self.address})
        print(f"Response Status Code: {response.status_code}")
        print(f"Response Data: {response.json()}")
        self.assertEqual(response.status_code, 401)
        
    def test_get_building_details_with_valid_token(self):
        print("Testing get_building_details_with_valid_token...")
        address_details_url = f"{self.base_url}_details"
        response = requests.post(address_details_url, headers=self.headers, json={"address": self.address})
        address_data = response.json()
        building_id = address_data['adgangsadresse']['id']

        building_details_url = f"{self.base_url}_details"
        building_response = requests.post(building_details_url, headers=self.headers, json={"address_id": building_id})
        print(f"Building Response Status Code: {building_response.status_code}")
        print(f"Building Response Data: {building_response.json()}")
        self.assertEqual(building_response.status_code, 200, "Failed to get building details with building id")
